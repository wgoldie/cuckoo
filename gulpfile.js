...
gulp.task('build-copytwcreds', function() {
    return gulp.src(['twcreds'])
        .pipe(gulp.dest('temp'));
});
...
gulp.task('build-ziptemp', function() {
    return gulp.src('temp/**/*', {nodir: true})
        .pipe(zip(buildname + '.zip'))
        .pipe(gulp.dest('dist/'));
});

gulp.task('buildzip', function() {
    return runSequence('build-copysrcfiles', 'build-copytwcreds', 'build-copynodemodules', 'build-ziptemp');
});
